...
static libspectrum_error
pzx_read_data(libspectrum_context_t *context, const libspectrum_byte **ptr, const libspectrum_byte *end,
                 ...
                 pzx_context *ctx )
{
  ...
  if( count ) {
    libspectrum_tape_block* block =
      libspectrum_tape_block_alloc( tape->context, LIBSPECTRUM_TAPE_BLOCK_ARCHIVE_INFO );
                 ...
                 pzx_context *ctx )
{
  ...
  block = libspectrum_tape_block_alloc( tape->context, LIBSPECTRUM_TAPE_BLOCK_DATA_BLOCK );
...
static libspectrum_error
read_next_pulse( libspectrum_context_t *context, const libspectrum_byte **buffer, const libspectrum_byte *end,
                 ...
                 pzx_context *ctx )
{
  ...
  while( ( block_end - (*buffer) ) > (ptrdiff_t)0 ) {
    error = read_next_pulse( tape->context, buffer, block_end, &pulse_repeats, &length );
    if( error ) {
  ...
  block = libspectrum_tape_block_alloc( tape->context, LIBSPECTRUM_TAPE_BLOCK_PULSE_SEQUENCE );
...
static libspectrum_error
read_block_header( libspectrum_context_t *context, char *id, libspectrum_dword *data_length,
		   const libspectrum_byte **buffer,
		   const libspectrum_byte *end )
{
  if( end - *buffer < 8 ) {
