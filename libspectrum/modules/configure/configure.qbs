...
*/}.toString().slice(15,-4));
    ...
    Rule {
        inputs: ['accessor.pl', 'snap_accessors.txt']

        Artifact {
            fileTags: 'c'
            filePath: 'snap_accessors.c'
        }

        prepare: {
            var cmd = new Command();
            var baseDir = FileInfo.path(input.filePath) + '/';
            cmd.description = 'generating snap_accessors.c';
            cmd.highlight = 'codegen';
            cmd.program=product.moduleProperty("configure", "PERL_PATH");
            cmd.arguments= [baseDir + 'accessor.pl', baseDir + 'snap_accessors.txt'];
            cmd.workingDirectory = baseDir;
    ...
    Rule {
        inputs: ['tape_accessors.pl', 'tape_accessors.txt']

        Artifact {
            fileTags: 'c'
            filePath: 'tape_accessors.c'
        }

        prepare: {
            var cmd = new Command();
            var baseDir = FileInfo.path(input.filePath) + '/';
            cmd.description = 'generating tape_accessors.c';
            cmd.highlight = 'codegen';
            cmd.program=product.moduleProperty("configure", "PERL_PATH");
            cmd.arguments= [baseDir + 'tape_accessors.pl', baseDir + 'tape_accessors.txt'];
            cmd.workingDirectory = baseDir;
            cmd.stdoutFilePath = output.filePath;
            return cmd;
        }
    ...
    Rule {
        inputs: ['tape_set.pl', 'tape_accessors.txt']
        ...
        prepare: {
            ...
            cmd.highlight = 'codegen';
            cmd.program=product.moduleProperty("configure", "PERL_PATH");
            cmd.arguments= [baseDir + 'tape_set.pl', baseDir + 'tape_accessors.txt'];
            cmd.workingDirectory = baseDir;
            cmd.stdoutFilePath = output.filePath;
            return cmd;
        }
